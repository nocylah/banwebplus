...
class user_ajax {
	...
	public static function create_user() {
		...
		mail($s_email, 'beanwebplus account', 'You just created an account on beanweb.plus with the username "'.$s_username.'."
Log in to your new account from www.beanweb.plus.

If you ever forget your password you can reset it from the main page by clicking on the "forgot password" link.', 'From: noreply@beanweb.plus');
		return json_encode(array(
	...
	public static function forgot_password($s_username = "", $s_email = "") {
		...
		$s_body = "A password reset attempt has been made with beanweb.plus for the user {$s_username}, registered with this email address. If you did not request this reset please ignore this email.\n\nYou have {$i_reset_minutes} minutes to click the link below to reset your password. Ignore this email if you do not want your password reset.\nhttp://beanweb.plus/pages/users/reset_password.php?username={$s_username}&key={$s_reset_key}";
		error_log($s_body);
		mail($s_email, "Request to Reset Banwebplus Password", $s_body, "From: noreply@beanweb.plus");
